...
- (void)addTopEdge:(CGSize)size lightSource:(NSInteger)lightSource toPath:(CGMutablePathRef)path
	...
	if (lightSource >= 0 && lightSource <= 90) {
		if (reset) {
			CGPathMoveToPoint(path, nil, TTRadius(_radius), 0);
		}
	} else {
		if (reset) {
			CGPathMoveToPoint(path, nil, 0, TTRadius(_radius));
		}
		CGPathAddArcToPoint(path, nil, 0, 0, TTRadius(_radius), 0, TTRadius(_radius));
	}
	...
	CGPathAddArcToPoint(path, nil, fw, 0, fw, TTRadius(_radius), TTRadius(_radius));
}
...
- (void)addRightEdge:(CGSize)size lightSource:(NSInteger)lightSource toPath:(CGMutablePathRef)path
	...
	if (reset) {
		CGPathMoveToPoint(path, nil, fw, TTRadius(_radius));
	}
	
	CGPathAddArcToPoint(path, nil, fw, fh, fw-TTRadius(_radius), fh, TTRadius(_radius));
}
...
- (void)addBottomEdge:(CGSize)size lightSource:(NSInteger)lightSource toPath:(CGMutablePathRef)path
	...
	if (reset) {
		CGPathMoveToPoint(path, nil, fw-TTRadius(_radius), fh);
	}
	...
	if (_pointLocation >= 225 && _pointLocation <= 315) {
		...
		CGPathAddArcToPoint(path, nil, fw, fh, floor(fw/2), fh, TTRadius(_radius));
		CGPathAddLineToPoint(path, nil, pointX+floor(_pointSize.width/2), fh);
		CGPathAddLineToPoint(path, nil, pointX, fh-ph);
		CGPathAddLineToPoint(path, nil, pointX-floor(_pointSize.width/2), fh);
		CGPathAddLineToPoint(path, nil, TTRadius(_radius), fh);
	}
	
	CGPathAddArcToPoint(path, nil, 0, fh, 0, fh-TTRadius(_radius), TTRadius(_radius));
}
...
- (void)addLeftEdge:(CGSize)size lightSource:(NSInteger)lightSource toPath:(CGMutablePathRef)path
	...
	if (reset) {
		CGPathMoveToPoint(path, nil, 0, fh-TTRadius(_radius));
	}
	
	if (lightSource >= 0 && lightSource <= 90) {
		CGPathAddArcToPoint(path, nil, 0, 0, TTRadius(_radius), 0, TTRadius(_radius));
	} else {
		CGPathAddLineToPoint(path, nil, 0, TTRadius(_radius));
	}
}
